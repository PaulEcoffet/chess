    ...
    def peutRoquer(self, sens):
        ...
        if isinstance(tour, Tour):
            if self.moves == 0 and tour.moves == 0:
                for col in smart_range(self.col + sens, col_a + sens):
                    if self.p.getPiece(self.line, col) is not None or self.echec(self.line, col):
                        return False
                    return True
        return False
    
    def echec(self, line=None, col=None):
        return False
